...
contract GiveAway is ReentrancyGuard {
    ...
    struct Criterial {
        ...
        mapping(bool => address) eligibleAddresses1;
    ...
    function addCriterial(
        ...
        for (uint256 i = 0; i < _eligibleAddresses.length; i++) {
            ...
            criterial.eligibleAddresses1[true] = _eligibleAddresses[i];
        }
    ...
    function getCriterial(uint256 idCriterial) public view returns (uint256 amount) {
    ...
    function getEligibleCriteriaCount(address _walletAddress) public view returns (uint256) {
        ...
        function getCriterial(uint256 idCriterial) public view returns (uint256 unitReward, address[] memory eligibleAddresses) {
        require(criterials[idCriterial].unitReward > 0, "Criterial does not exist");
    ...
    function getCriterialEligibleAddresses(uint256 idCriterial) internal view returns (address[] memory addresses) {
        ...
        uint256 count;
        for (uint256 i = 0; i < nextCriterialId; i++) {
            if (criterial.eligibleAddresses[criterial.eligibleAddresses1[true]]) {
                count++;
            }
        }
        ...
        for (uint256 i = 0; i < nextCriterialId; i++) {
            if (criterial.eligibleAddresses[criterial.eligibleAddresses1[true]]) {
                addresses[j] = criterial.eligibleAddresses1[true];
                j++;
            }
        }
        return addresses;
