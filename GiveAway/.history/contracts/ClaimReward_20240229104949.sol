...
contract GiveAway is ReentrancyGuard {
    ...
    function _getCriteria(uint256 criterialId) internal view returns (Criterial memory) {
    require(criterials[criterialId].unitReward > 0, "Criterial does not exist");
    return criterials[criterialId];
}

function getUnitReward(uint256 criterialId) public view returns (uint256) {
    Criterial memory criterial = _getCriteria(criterialId);
    return criterial.unitReward;
}

function getEligibleAddressesCount(uint256 criterialId) public view returns (uint256) {
    Criterial memory criterial = _getCriteria(criterialId);
    return criterial.eligibleAddresses.length;
}
