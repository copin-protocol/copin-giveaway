...
contract GiveAway is ReentrancyGuard {
    ...
    struct Criterial {
        uint256 unitReward;
        uint8 idCriterial;
        mapping(uint => mapping(address => bool)) eligibleAddresses;
    ...
    function getCriterial(uint8 idCriterial) public view returns(uint amount){
      Criterial storage criterials = criterials[nextCriterialId];
      return criterials.eligibleAddresses.length
    }
