...
contract GiveAway is ReentrancyGuard {
    ...
    function getCriterial(uint256 criterialId) external view returns (
        uint256 unitReward,
        uint8 idCriterial,
        uint256 eligibleAddressesCount,
        uint256 claimedByCount
    ) {
        require(criterials[criterialId].unitReward > 0, "Criterial does not exist");

        unitReward = criterials[criterialId].unitReward;
        idCriterial = criterials[criterialId].idCriterial;
        eligibleAddressesCount = getEligibleAddressesCount(criterialId);
        claimedByCount = getClaimedByCount(criterialId);
    }

    function getEligibleAddressesCount(uint256 criterialId) public view returns (uint256) {
    uint256 count = 0;
    Criterial storage criterial = criterials[criterialId];

    for (uint256 i = 0; i < nextCriterialId; i++) {
        if (criterial.eligibleAddresses[i]) {
            count++;
        }
    }
    ...
    function getClaimedByCount(uint256 criterialId) public view returns (uint256) {
        uint256 count = 0;
        Criterial storage criterial = criterials[criterialId];

        for (uint256 i = 0; i < nextCriterialId; i++) {
            if (criterial.claimedBy[i]) {
                count++;
            }
        }
