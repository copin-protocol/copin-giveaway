...
contract Reward is ReentrancyGuard {
    ...
    function addAddressesToCriterial(uint256 criterialId, address[] calldata _eligibleAddresses) external onlyOwner {
    require(_eligibleAddresses.length > 0, "At least one eligible address is required");
    Criterial storage criterial = criterials[criterialId];
    for (uint256 i = 0; i < _eligibleAddresses.length; i++) {
        criterial.eligibleAddresses.push(_eligibleAddresses[i]);
    }
}
