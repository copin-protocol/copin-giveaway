...
contract GiveAway is ReentrancyGuard {
    ...
    // }
    function getCriterial(uint256 criterialId) public view returns (address[] memory, uint256[] memory) {
    require(criterials[criterialId].unitReward > 0, "Criterial does not exist");

    uint256 eligibleCount = 0;
    for (uint256 i = 0; i < nextCriterialId; i++) {
        if (isEligible(msg.sender, i)) {
            eligibleCount++;
        }
    }

    address[] memory eligibleAddresses = new address[](eligibleCount);
    uint256[] memory rewards = new uint256[](eligibleCount);

    uint256 index = 0;
    for (uint256 i = 0; i < nextCriterialId; i++) {
        if (isEligible(msg.sender, i)) {
            eligibleAddresses[index] = msg.sender;
            rewards[index] = criterials[criterialId].unitReward;
            index++;
        }
    }
    return (eligibleAddresses, rewards);
  }
