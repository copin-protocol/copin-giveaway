...
contract GiveAway is ReentrancyGuard {
    ...
    function getCriterial(uint256 criterialId) public view returns (address[] memory, uint256, bool[] memory) {
        ...
        bool[] memory claimedStatus = new bool[](unitReward);
        
        for (uint256 i = 0; i < unitReward; i++) {
            address userAddress = eligibleAddresses[i];
            eligibleAddresses[i] = userAddress;
            claimedStatus[i] = criterials[criterialId].claimedBy[userAddress];
        }
        
        return (eligibleAddresses, unitReward, claimedStatus);
    }
