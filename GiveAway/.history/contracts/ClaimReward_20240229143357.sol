...
contract GiveAway is ReentrancyGuard {
    ...
    mapping(uint256 => address[]) public criterialAddresses;
    ...
    function getCriterial(uint256 idCriterial) public view returns (uint256 unitReward, uint256 numberOfAddresses, address[] memory eligibleAddresses) {
        require(criterials[idCriterial].unitReward > 0, "Criterial does not exist");

        return (
            criterials[idCriterial].unitReward,
            criterials[idCriterial].numberOfAddresses,
            getCriterialEligibleAddresses(idCriterial)
        );
    }

    function getCriterialEligibleAddresses(uint256 idCriterial) internal view returns (address[] memory addresses) {
        // Lấy thông tin của tiêu chí dựa trên idCriterial
        Criterial storage criterial = criterials[idCriterial];

        // Khởi tạo mảng địa chỉ có kích thước là số lượng địa chỉ phê duyệt trong tiêu chí
        addresses = new address[](criterial.numberOfAddresses);

        // Biến index để theo dõi vị trí hiện tại trong mảng addresses
        uint256 index = 0;

        // Duyệt qua tất cả các id tiêu chí
        for (uint256 i = 0; i < criterial.numberOfAddresses; i++) {
            // Nếu địa chỉ có thứ tự i trong mapping eligibleAddresses của tiêu chí được phê duyệt
            if (criterial.eligibleAddresses[criterial.eligibleAddresses1[i]]) {
                // Thêm địa chỉ đó vào mảng addresses và tăng index
                addresses[index] = criterial.eligibleAddresses1[i];
                index++;
            }
        }

        // Trả về mảng addresses chứa danh sách các địa chỉ được phê duyệt cho tiêu chí
        return addresses;
    }
