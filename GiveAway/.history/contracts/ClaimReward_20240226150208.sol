...
//     mapping(uint256 => Criterial) public criterials;
//     mapping(address => mapping(uint256 => bool)) public userEligibleForCriterial;
...
//             userEligibleForCriterial[_eligibleAddresses[i]][nextCriterialId] = true;
//         }
//         criterial.unitReward = unitReward;
//         nextCriterialId++;
//     }
...
//         uint256 criterialId
//     ) public view returns (bool) {
//         return userEligibleForCriterial[user][criterialId];
//     }
...
//         uint256 count = 0;
//         for (uint256 i = 1; i < nextCriterialId; i++) {
//             if (userEligibleForCriterial[_walletAddress][i]) {
//                 count++;
//             }
//         }
//         return count;
...
//     //     Criterial storage criterial = criterials[criterialId];
//     //     return (
//     //         getCriterialEligibleAddresses(criterialId),
//     //         criterial.unitReward
//     //     );
//     // }

//     // function getCriterialEligibleAddresses(uint256 criterialId) internal view returns (address[] memory) {
//     //     Criterial storage criterial = criterials[criterialId];
//     //     address[] memory eligibleAddresses = new address[](criterialId);
//     //     uint256 count = 0;
//     //     for (uint256 i = 0; i < criterialId; i++) {
//     //         if (criterial.eligibleAddresses[address(i)]) {
//     //             eligibleAddresses[count] = address(i);
//     //             count++;
//     //         }
//     //     }
...
contract GiveAway is ReentrancyGuard {
    ...
    function addCriterial(
        ...
        for (uint256 i = 0; i < _eligibleAddresses.length; i++) {
            criterial.eligibleAddresses[_eligibleAddresses[i]] = true;
            userEligibleForCriterial[_eligibleAddresses[i]][nextCriterialId] = true;
            emit AddressAdded(_eligibleAddresses[i], nextCriterialId);
        }
        criterial.unitReward = unitReward;
        nextCriterialId++;
    }
    ...
    function removeAddress(address _address, uint256 criterialId) public onlyOwner {
        ...
        delete userEligibleForCriterial[_address][criterialId];
    ...
    function getCriterial(uint256 criterialId) public view returns (uint256, address[] memory) {
        // Retrieve eligible addresses efficiently
        address[] memory eligibleAddresses = new address[](countEligibleAddresses(criterialId));
        uint256 j = 0;
        for (address addr in userEligibleForCriterial) {
            if (userEligibleForCriterial[addr][criterialId]) {
                eligibleAddresses[j] = addr;
                j++;
            }
        }
        return (criterials[criterialId].unitReward, eligibleAddresses);
    }
    function getAddress() public view returns (address) {
        return address(this);
    }
