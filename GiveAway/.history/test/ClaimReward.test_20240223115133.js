...
const {} =  require("@ethersproject/strings")
describe("Reward contract", function () {
  ...
  it("Should add a criterial with eligible addresses", async function () {
    ...
    expect(criterial[0]).to.equal(keccak256(utils.toUtf8Bytes(label)));
  ...
  it("Should add 200 addresses to a criterial using addCriterial", async function () {
    const { rewardContract, addr1 } = await loadFixture(deployRewardFixture);
