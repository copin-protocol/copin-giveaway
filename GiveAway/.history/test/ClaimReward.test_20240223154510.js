...
describe("Reward contract", function () {
  ...
  it("Should allow eligible user to claim reward", async function () {
    const { rewardContract, addr1, RewardPerCriterial } = await loadFixture(
      deployRewardFixture
    );
    ...
    expect(countBefore).to.equal(1, "Incorrect count of eligible criteria for addr1");
    ...
    expect(countAfter).to.equal(0, "Incorrect count of eligible criteria for addr1 after claiming");
    console.log(1);
    // Check claimed rewards for addr1
    const claimedAmount = await rewardContract.claimedRewards(addr1.address);
    console.log(1);
    expect(claimedAmount).to.equal(RewardPerCriterial, "Incorrect claimed rewards for addr1");

    // Check balance of addr1 after claiming
    const balanceAfter = await rewardTokenContract.balanceOf(addr1.address);
    expect(balanceAfter).to.equal(RewardPerCriterial, "Incorrect balance for addr1 after claiming");
  });
