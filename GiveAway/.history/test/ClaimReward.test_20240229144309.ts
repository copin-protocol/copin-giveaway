...
describe("Reward contract", function () {
  ...
  //   expect(returnedAddresses).to.equal([addr1.address, addr2.address]);
  // });
  ...
  it("Should revert if Criterial does not exist to getCriterial", async function () {
    ...
    await rewardContract.addCriterial([addr1.address,addr2.], unitReward);
    
    console.log(await rewardContract.getCriterial(1));
  });
});