...
describe("Reward contract", function () {
  ...
  it("Should add a criterial with eligible addresses", async function () {
    const { rewardContract, addr1,unitReward } = await loadFixture(deployRewardFixture);
  ...
  it("Should revert if trying to add a criterial with unit reward less than or equal to 0 ", async function () {
    ...
    await expect(rewardContract.addCriterial([addr1.address], 0)).to.be.revertedWith("Unit reward must be greater than 0");
  });
  ...
  it("Should revert if no eligible addresses are provided when adding a criterial ", async function () {
    ...
    await expect(rewardContract.addCriterial([], 123)).to.be.revertedWith("At least one eligible address must be provided");
  });
  //============================================================================================================
  // addAddressToCriterial

  it("Should additional eligible addresses into criterial", async function () {
    const { rewardContract, unitReward, addr1, addr2 } = await loadFixture(deployRewardFixture);
  ...
  it("Should revert if Criterial does not exist ", async function () {
    ...
    await expect(rewardContract.addAddressToCriterial([addr2.address], 2)).to.be.revertedWith("Criterial does not exist");
  });
  it("Should revert if no eligible addresses are provided when adding a criterial ", async function () {
    const { rewardContract , addr1 ,unitReward} = await loadFixture(deployRewardFixture);
    ...
    await expect(rewardContract.addAddressToCriterial([], 1)).to.be.revertedWith("At least one eligible address must be provided");
  });
  //============================================================================================================
  // transfer
  ...
  it("Should allow 200 eligible users to claim reward", async function () {
    const { rewardContract, rewardTokenContract, total,unitReward } = await loadFixture(
      deployRewardFixture
    );
  ...
  it("Should revert if reward already claimed", async function () {
    const { rewardContract, rewardTokenContract, total, addr1,unitReward } = await loadFixture(
      deployRewardFixture
    );
  ...
  it("Should revert if contract does not have enough balance to transfer to the claimer", async function () {
    const { rewardContract,rewardTokenContract ,addr1, addr2 } = await loadFixture(deployRewardFixture);

    // Add a criterial with a unit reward of 100 tokens
    await rewardContract.addCriterial([addr1.address], 100);

    // Transfer 50 tokens to the contract
    await rewardTokenContract.transfer(rewardContract.address, 50);

    // Claim the reward for criterial 1, should revert because contract does not have enough balance
    await expect(rewardContract.connect(addr1).claimReward(1)).to.be.revertedWith("Not enough balance");
});
