...
describe("Reward contract", function () {
  ...
  it("Should return eligible addresses and unit reward for existing criterial", async function () {
    ...
    const { eligibleAddresses: criterialAddresses, unitReward: returnedReward } = await rewardContract.getCriterial(1);
    
    // expect(returnedReward).to.equal(unitReward);
    expect(criterialAddresses).to.equal([addr1.address, addr2.address]);
  });
