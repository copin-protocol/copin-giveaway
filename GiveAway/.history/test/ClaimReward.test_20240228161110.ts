...
describe("Reward contract", function () {
  ...
  it("Should revert if Unit reward must be greater than 0 ", async function () {
    ...
    await rewardContract.addCriterial([addr1.address], 0);
  
    // Try to remove addr2 which is not eligible from eligible Addresses
    await expect(rewardContract.addAddressToCriterial([addr2.address], 2)).to.be.revertedWith("Unit reward must be greater than 0");
  });
