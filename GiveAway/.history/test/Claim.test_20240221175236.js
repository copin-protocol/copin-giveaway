...
describe("Token contract", function () {
  ...
  it("Should allow eligible user1 for Criterial -Have a girl", async function () {
    ...
    await claimReward.addCriterial(label, [addr1.address], 123);
    const bool = await claimReward.isEligible(addr1.address, 1);
    expect(bool).to.equal(true);
  });
  it("Show all Criterial", async function () {
    const { claimReward } = await loadFixture(
      deployTokenFixture
    ); 
