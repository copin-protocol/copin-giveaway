...
describe("Reward contract", function () {
  ...
  it("Should initialize contract with correct initial values", async function () {
    // Deploy the GiveAway contract
    const rewardCoinAddress = // Your reward coin address;
    const { rewardContract } = await loadFixture(deployRewardFixture, rewardCoinAddress);

    // Verify that the contract is initialized with the correct reward coin address
    expect(await rewardContract.I_Optimism()).to.equal(rewardCoinAddress);

    // Verify that the contract is initialized with the correct owner address
    expect(await rewardContract.owner()).to.equal(ownerAddress);

    // Verify that the contract is initialized with the correct initial value for nextCriterialId
    expect(await rewardContract.nextCriterialId()).to.equal(1);
});
