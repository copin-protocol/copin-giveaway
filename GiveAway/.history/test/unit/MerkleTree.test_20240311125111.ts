...
describe("Reward contract", function () {
  describe("Deployment", function () {
    ...
    it("Should address 1 claimReward ", async function () {
      const { rewardTokenContract, merkletreeContract,tree,wallets,addr_merkletreeContrac } =
        await loadFixtureToolbox(fixture);
      ...
      const index = 0;
      const amount = 11111;
      console.log(await rewardCoinAddress.balanceOf(addr_merkletreeContrac));
      expect(await rewardTokenContract.balanceOf(wallets[].address)).to.equal(
        0
      );
