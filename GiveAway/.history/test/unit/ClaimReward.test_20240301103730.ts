...
describe("Reward contract", function () {
  describe("Deployment", function () {
    ...
    it("Should revert if not enough gas to add criterial", async function () {
      // Ước tính chi phí gas cho việc thêm tiêu chí
    const numAddresses = 1; // Đặt số lượng địa chỉ hợp lệ
    const unitReward = 100; // Đặt phần thưởng đơn vị
    const estimatedGasCost = await rewardContract.estimateGas.addCriterial([wallets[1].address], unitReward);

    // Đặt giới hạn gas ít hơn so với chi phí gas ước tính
    const gasLimit = estimatedGasCost - 1000; // Trừ một số giá trị để đảm bảo giới hạn gas ít hơn

    // Cố gắng thêm tiêu chí với giới hạn gas được đặt ít hơn so với chi phí gas ước tính
    await expect(
        rewardContract.connect(wallets[0]).addCriterial([wallets[1].address], unitReward, { gasLimit })
