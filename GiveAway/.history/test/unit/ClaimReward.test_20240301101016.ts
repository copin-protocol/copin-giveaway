  ...
  describe("Deployment", function () {
    ...
    describe("AddCriterial", function () {
      it("Should allow only owner to add criterial", async function () {
        await expect(
          rewardContract.connect(wallets[1]).addCriterial([wallets[1].address], 100)
    ...
    it("Should revert if not enough gas to add criterial", async function () {
      // Estimate gas cost for adding criterial with a large number of eligible addresses
      const numAddresses = 100; // Set number of eligible addresses
      const addresses = Array.from({ length: numAddresses }, (_, i) => wallets[i].address);
      const unitReward = 100; // Set unit reward
      const estimatedGasCost = await rewardContract.estimateGas.addCriterial(addresses, unitReward);

      // Set gas limit less than the estimated gas cost
      const gasLimit = estimatedGasCost - 1000; // Subtract some value to ensure gas limit is less

      // Attempt to add criterial with gas limit set to less than the estimated gas cost
      await expect(
          rewardContract.connect(wallets[0]).addCriterial(addresses, unitReward, { gasLimit })
      ).to.be.rejectedWith("Transaction reverted: out of gas");
  });
