...
describe("Reward contract", function () {
  describe("Deployment", function () {
    ...
    const fixture = async () => {
      ...
      const merkletreeContract = await MerkleTreeContract.deploy(rewardTokenContract,tree.root);
    ...
    it("Should initialize contract with correct initial values", async function () {
      ...
      expect(await merkletreeContract.merkleRoot).to.equal(wallets[0].address);
    });
  });
});