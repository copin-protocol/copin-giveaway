...
describe("Reward contract", function () {
  describe("Deployment", function () {
    ...
    const fixture = async (wallets: SignerWithAddress[]) => {
      const { rewardContract,rewardTokenContract,total } = await completeFixture(wallets);
      unitReward = 100;
      return {
        rewardContract,
        rewardTokenContract,
    ...
    beforeEach("load fixture", async () => {
      ({ unitReward,rewardContract,rewardTokenContract,total } = await loadFixture(fixture));
    });
    it("Should initialize contract with correct initial values", async function () {
      ...
      expect(await rewardContract.owner()).to.equal(wallets.address);
