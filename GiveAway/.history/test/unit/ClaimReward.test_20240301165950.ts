  ...
  describe("Deployment", function () {
    ...
    describe("AddCriterial", function () {
      ...
      it("Should add a criterial with 200 eligible addresses", async function () {
        ...
        for (let i = 0; i < 200; i++) {
          expect(await rewardContract.isEligible(addresses[i], 1)).to.be.equal(
            true
          );
        }
      });
      ...
      it("Should revert if not enough gas to add criterial", async function () {
        ...
        const estimatedGasCost = Number((await tx.wait()).gasUsed);
      ...
      it("Should revert if not enough gas to add criterial with 200 eligible addresses", async function () {
        const addrRewardContract = await rewardContract.getAddress();
        ...
        await Promise.all(addresses);
        const tx = await rewardContract.addCriterial(addresses, unitReward);
        ...
        await expect(
          rewardContract
            .connect(wallets[0])
            .addCriterial(addresses, unitReward, { gasLimit })
        ).to.be.rejectedWith("out of gas");
      });
    });

    //============================================================================================================
    // AdditionalToCriterial

    describe("Additional Addresses", function () {
      it("Should allow only owner to AdditionalToCriterial", async function () {
        await rewardContract.addCriterial([wallets[2].address], unitReward);
        await expect(
          rewardContract
            .connect(wallets[1])
            .AdditionalToCriterial([wallets[1].address], 1)
        ).to.be.rejectedWith("Only owner can call this function");
      ...
      it("Should additional eligible addresses into criterial", async function () {
        await rewardContract.addCriterial([wallets[1].address], unitReward);
        await rewardContract.AdditionalToCriterial([wallets[2].address], 1);
      ...
      // it("Should additional 200 eligible addresses into criterial", async function () {
      //   await rewardContract.addCriterial([wallets[1].address], unitReward);

      //   const eligibleAddresses = wallets.slice(2, 202);
      //   const addresses: string[] = [];
      ...
      //   await Promise.all(addresses);
      //   await rewardContract.AdditionalToCriterial([addresses], 1);

      //   const newAddresses = addresses.push(wallets[1].address);
      //   for (let i = 0; i < 200; i++) {
      //     expect(await rewardContract.isEligible(newAddresses[i], 1)).to.be.equal(
      //       true
      //     );
      //   }
      // });
      ...
      it("Should revert if Criterial does not exist ", async function () {
        ...
        await expect(
          rewardContract.AdditionalToCriterial([wallets[2].address], 2)
        ).to.be.rejectedWith("Criterial does not exist");
      });
      ...
      it("Should revert if no eligible addresses are provided when adding a criterial ", async function () {
        ...
        await expect(
          rewardContract.AdditionalToCriterial([], 1)
        ).to.be.rejectedWith("At least one eligible address must be provided");
      });

      it("Should revert if not enough gas to Additional Addresses", async function () {
        await rewardContract.addCriterial([wallets[1].address], unitReward);
        ...
        await expect(
          rewardContract
            .connect(wallets[0])
            .AdditionalToCriterial([wallets[2].address], 1, { gasLimit })
        ).to.be.rejectedWith("but got 1000"); // Using a regular expression to match any message containing "out of gas"
      });
    });
    ...
    describe("Remove address", function () {
      it("Should Remove address 2 from eligible Addresses", async function () {
        ...
        expect(
          await rewardContract.isEligible(wallets[2].address, 1)
        ).to.deep.equal(false);
        expect(
          await rewardContract.isEligible(wallets[1].address, 1)
        ).to.deep.equal(true);
      });
      ...
      it("Should revert if non-owner tries to remove address", async function () {
        ...
        await expect(
          rewardContract
            .connect(wallets[1])
            .removeAddress(wallets[2].address, 1)
        ).to.be.rejectedWith("Only owner can call this function");
      });

      it("Should revert if not enough gas to remove", async function () {
        const tx = await rewardContract.addCriterial(
          [wallets[1].address, wallets[2].address],
          unitReward
        );
        ...
        await expect(
          rewardContract.removeAddress(wallets[2].address, 1, { gasLimit })
    ...
    describe("Claim Reward", function () {
      ...
      it("Should revert if not enough gas to Claim Reward", async function () {
        ...
        await expect(
          rewardContract.connect(wallets[1]).claimReward(1, { gasLimit })
        ).to.be.rejectedWith("but got 1000");

        await expect(
          rewardContract.connect(wallets[2]).claimReward(1, { gasLimit })
        ).to.be.rejectedWith("but got 1000");
      });
    });
    ...
    it("Should revert if not enough gas to Claim Reward with 200 eligible addresses", async function () {
      ...
      for (let i = 0; i < 200; i++) {
        try {
            if (i < 190) {
                claims.push(await rewardContract.connect(eligibleAddresses[i]).claimReward(1));
            } else {
                await rewardContract.connect(eligibleAddresses[i]).claimReward(1, { gasLimit });
                // Nếu không có lỗi, chạy dòng này sẽ không bị lỗi, do đó chúng ta cần kiểm tra một cách tường minh rằng nó đã bị lỗi
                throw new Error("Expected to revert with out of gas");
            }
        } catch (error) {
    ...
    describe("Check userClaimed", function () {
      it("Should return true if user claimed reward", async function () {
        ...
        expect(
          await rewardContract.userClaimed(wallets[1].address, 1)
        ).to.be.equal(true);
      });
