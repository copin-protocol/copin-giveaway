...
describe("Reward contract", function () {
  describe("Deployment", function () {
    ...
    describe("Additional Addresses", function () {
      ...
      //   await Promise.all(addresses);
      //   await rewardContract.addAddressToCriterial([addresses], 1);
      ...
      //     unitReward
      //   );
      //   const tx = await rewardContract.addAddressToCriterial([wallets[2]], 1)
      //   const estimatedGasCost = Number((await tx.wait()).gasUsed);
      ...
      //     rewardContract
      //       .connect(wallets[0])
      //       .addAddressToCriterial([wallets[2].address], 1, { gasLimit })
      //   ).to.be.rejectedWith("out of gas");
      // });
    });
    ...
    describe("Remove address", function () {
      ...
      it("Should revert if not enough gas to remove", async function () {
        ...
        await expect(
          rewardContract
            .connect(wallets[0])
            .removeAddress(wallets[2], unitReward, { gasLimit })
        ).to.be.rejectedWith("out of gas");
      });
    });
