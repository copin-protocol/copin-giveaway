...
describe("Reward contract", function () {
  describe("Deployment", function () {
    ...
    it("Should revert if not enough gas to add criterial", async function () {
      try {
        // Set a very low gas limit to intentionally cause a gas shortage
        const tx = await rewardContract.addCriterial([wallets[1].address], 100, { gasLimit: 21000 }); // Adjust gasLimit as needed
        await tx.wait(); // This will throw an error as the transaction will revert
      } catch (error) {
        expect(error.message).to.contain("Out of gas");
      }
    });
