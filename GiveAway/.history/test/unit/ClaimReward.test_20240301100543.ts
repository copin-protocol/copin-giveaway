...
describe("Reward contract", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should revert if not enough gas to add criterial", async function () {
      // Số lượng địa chỉ hợp lệ lớn
      const largeNumberOfAddresses = new Array(1000).fill(wallets[1].address);

      // Giả định một số lượng gas không đủ để thêm bộ tiêu chí mới
      const insufficientGas = ethers.BigNumber.from("100000");

      // Kiểm tra xem hàm addCriterial có bị rejected khi không đủ gas hay không
      await expect(
        rewardContract.addCriterial(largeNumberOfAddresses, unitReward, { gasLimit: insufficientGas })
      ).to.be.rejectedWith("Transaction reverted: not enough gas");
    });
