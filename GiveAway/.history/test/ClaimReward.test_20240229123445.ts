...
describe("Reward contract", function () {
  ...
  it("Should return eligible addresses and unit reward for existing criterial", async function () {
    const { rewardContract ,addr1,addr2 } = await loadFixture(deployRewardFixture);
    
    // Add a criterial with some eligible addresses
    const eligibleAddresses = [address1, address2, address3];
    const unitReward = 100;
    await rewardContract.addCriterial(eligibleAddresses, unitReward);
    
    // Get the criterial details
    const { eligibleAddresses: returnedAddresses, unitReward: returnedReward } = await rewardContract.getCriterial(1);
    
    // Check if the returned details match the added criterial
    expect(returnedReward).to.equal(unitReward);
    expect(returnedAddresses).to.have.lengthOf(eligibleAddresses.length);
    for (let i = 0; i < eligibleAddresses.length; i++) {
      expect(returnedAddresses[i]).to.equal(eligibleAddresses[i]);
    }
  });
  

  it("Should revert if Criterial does not exist to getCriterial", async function () {
    const { rewardContract } = await loadFixture(deployRewardFixture);
    
    expect(await rewardContract.getCriterial(1)).to.be.revertedWith(
