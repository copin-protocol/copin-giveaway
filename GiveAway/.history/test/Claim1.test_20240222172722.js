...
describe("Reward contract", function () {
  ...
  it("Should count eligible criteria correctly ", async function () {
    const { rewardContract, total, addr1, addr2, RewardPerCriterial } =
  ...
  it("Should add 200 addresses to a criterial use addAddressesToCriterial", async function () {
    const { rewardContract, addr1 } = await loadFixture(deployRewardFixture);

    // Add a criterial
    const label = "Test Criterial";
    const unitReward = 100;
    await rewardContract.addCriterial(label, [], unitReward);
    ...
    // // Calculate gas for adding addresses to the criterial
    // const addAddressesTx = await rewardContract.estimateGas.addAddressesToCriterial(1, addresses);
    // console.log("Gas used for adding addresses:", addAddressesTx.toNumber());

    // Add the addresses to the criterial
    const tx = await rewardContract.addAddressesToCriterial(1, addresses);
    const receipt = await tx.wait();

    const gasUsed = receipt.gasUsed;
    console.log("Gas used for adding addresses:", gasUsed);
    // Verify that the addresses were added correctly
    const criterial = await rewardContract.getCriterial(1);
    expect(criterial[1].length).to.equal(200);
  });
  it("Should add 200 addresses to a criterial use addCriterial", async function () {
    const { rewardContract, addr1 } = await loadFixture(deployRewardFixture);
    ...
    await rewardContract.addCriterial(label, addresses, unitReward);
    // // Calculate gas for adding addresses to the criterial
    // const addAddressesTx = await rewardContract.estimateGas.addAddressesToCriterial(1, addresses);
    // console.log("Gas used for adding addresses:", addAddressesTx.toNumber());
  ...
  //   await rewardContract.addCriterial(label, [addr1.address], RewardPerCriterial);
  //   // Claim reward for addr1
  //   await rewardTokenContract.connect(addrRewardContract).transferFrom(addrRewardContract, addr1.address, RewardPerCriterial);
