...
describe("Reward contract", function () {
  ...
  it("Should return eligible addresses and unit reward for an existing criterial", async function () {
    ...
    await rewardContract.addCriterial([ad, accounts[2]], 100);

    // Lấy thông tin của tiêu chí
    const [addresses, unitReward] = await rewardContract.getCriterial(1);

    // Kiểm tra xem số lượng địa chỉ và phần thưởng có đúng không
    expect(addresses.length).to.equal(2);
    expect(unitReward).to.equal(100);

    // Kiểm tra xem địa chỉ đầu tiên và thứ hai có đúng không
    expect(addresses[0]).to.equal(ad);
    expect(addresses[1]).to.equal(accounts[2]);
  });
