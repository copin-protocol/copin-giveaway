...
describe("Reward contract", function () {
  ...
  it("Should transfer all tokens from owner to Reward contract", async function () {
    ...
    // Get the balance of the Reward contract before and after the transfer
    const initialBalance = await rewardTokenContract.balanceOf(await rewardContract.getAddress());
    await rewardContract.connect(owner).transferFrom(owner.address, rewardContract.address, total);
    const finalBalance = await rewardTokenContract.balanceOf(rewardContract.address);
    console.log("Balance RWT in RewardContract : ",finalBalance);
    // Assert that the final balance equals the initial balance plus the transferred amount
    expect(finalBalance).to.equal(initialBalance.add(total));
  });
