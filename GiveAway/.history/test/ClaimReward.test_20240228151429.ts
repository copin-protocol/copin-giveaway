...
describe("Reward contract", function () {
  ...
  it("Should revert if Criterial does not exist ", async function () {
    ...
    await rewardContract.addCriterial([addr1.address], unitReward);
    await rewardContract.addAddressToCriterial([addr2.address], 2)
  
    // Try to remove addr2 which is not eligible from eligible Addresses
    await expect(rewardContract.addAddressToCriterial([addr2.address], 2)).to.be.revertedWith("Address is not eligible for this criterial");
  });
  //============================================================================================================
  // transfer
