...
describe("Token contract", function () {
  ...
  it("Show all eligible address for -Have a girlfriend-", async function () {
    const { claimReward } = await loadFixture(deployTokenFixture);
    const {criterial.label, criterial.eligibleAddresses, criterial.unitReward} = await claimReward.getCriterial(1);
