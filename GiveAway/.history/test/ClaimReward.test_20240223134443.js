...
describe("Reward contract", function () {
  ...
  it("Should allow eligible user to claim reward", async function () {
    const { rewardContract,rewardTokenContract, addr1, RewardPerCriterial } = await loadFixture(
      deployRewardFixture
    );

    // Add a criterial with addr1 as eligible
    await rewardContract.addCriterial("Test Criterial", [addr1.address], RewardPerCriterial);

    // Get count of eligible criteria for addr1
    const countBefore = await rewardContract.getEligibleCriteriaCount(addr1.address);
    assert.equal(countBefore, 1, "Incorrect count of eligible criteria for addr1");

    // addr1 claims reward
    await rewardContract.connect(addr1).claimReward(1, { value: RewardPerCriterial });

    // Get count of eligible criteria for addr1 after claiming
    const countAfter = await rewardContract.getEligibleCriteriaCount(addr1.address);
    assert.equal(countAfter, 0, "Incorrect count of eligible criteria for addr1 after claiming");

    // Check claimed rewards for addr1
    const claimedAmount = await rewardContract.claimedRewards(addr1.address);
    assert.equal(claimedAmount, RewardPerCriterial, "Incorrect claimed rewards for addr1");

    // Check balance of addr1 after claiming
    const balanceAfter = await rewardTokenContract.balanceOf(addr1.address);
    assert.equal(balanceAfter, RewardPerCriterial, "Incorrect balance for addr1 after claiming");
});
  ...
  //     RewardPerCriterial
  //   );
  //   await rewardContract.connect(addr1.address).claimReward(1);
