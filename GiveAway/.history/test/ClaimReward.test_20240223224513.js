...
describe("Reward contract", function () {
  ...
  it("Should add 200 addresses to a criterial and all claim rewards", async function () {
    const { rewardContract, rewardTokenContract } = await loadFixture(deployRewardFixture);
    ...
    for (let i = 0; i < addresses.length; i++) {
        ...
        const balanceBefore = await rewardTokenContract.balanceOf(address);
        await rewardContract.connect(ethers.provider.getSigner()).claimReward(1, { from: address });
        const balanceAfter = await rewardTokenContract.balanceOf(address);
        expect(balanceAfter.sub(balanceBefore)).to.equal(unitReward);
    }
});
