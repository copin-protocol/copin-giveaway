...
describe("Reward contract", function () {
  async function deployRewardFixture() {
    ...
    return { rewardContract, rewardTokenContract, owner, addr1, addr2 ,total};
  }
  ...
  it("Should transfer all tokens from owner to Reward contract", async function () {
    const { rewardContract, rewardTokenContract, owner, total } = await loadFixture(deployRewardFixture);
    const addrRewardContract = await  
    // Get the balance of the owner before and after the transfer
    const initialBalanceOwner = await rewardTokenContract.balanceOf(owner.address);
    const initialBalanceRewardContract = await rewardTokenContract.balanceOf(await rewardContract.getAddress);

    // Transfer all tokens from owner to Reward contract
    await rewardTokenContract.transfer(rewardContract.address, total);

    // Get the balance of the owner and Reward contract after the transfer
    const finalBalanceOwner = await rewardTokenContract.balanceOf(owner.address);
    const finalBalanceRewardContract = await rewardTokenContract.balanceOf(rewardContract.address);

    // Assert that the final balances are correct
    expect(finalBalanceOwner).to.equal(initialBalanceOwner.sub(total));
    expect(finalBalanceRewardContract).to.equal(initialBalanceRewardContract.add(total));
});
