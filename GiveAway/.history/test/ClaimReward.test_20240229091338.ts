...
describe("Reward contract", function () {
  ...
  it("Should allow only owner to add criterial", async function () {
    ...
    expect(await rewardContract.isEligible(addr1.address, 1)).to.be.revertedWith("Criterial does not exist");
  });
  ...
  it("Should revert if Criterial does not exist"), async function () {
    ...
    expect(await rewardContract.isEligible(addr1.address, 2)).to.be.revertedWith("Criterial does not exist");
  }
