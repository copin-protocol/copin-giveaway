...
describe("Reward contract", function () {
  ...
  it("Should revert if Unit reward must be greater than 0 ", async function () {
    const { rewardContract, addr1} = await loadFixture(deployRewardFixture);
  ...
  it("Should revert if At least one eligible address must be provided ", async function () {
    const { rewardContract, addr1} = await loadFixture(deployRewardFixture);
  
    // Try to remove addr2 which is not eligible from eligible Addresses
    await expect(rewardContract.addCriterial([addr1.address], 0)).to.be.revertedWith("At least one eligible address must be provided");
  });
  //============================================================================================================
  ...
  // it("Should revert if Address already exists in this criterial ", async function () {
  //   const { rewardContract, unitReward, addr1} = await loadFixture(deployRewardFixture);
    
  //   await rewardContract.addCriterial([addr1.address], unitReward);
  
  //   // Try to remove addr2 which is not eligible from eligible Addresses
  //   await expect(rewardContract.addAddressToCriterial([addr1.address], 1)).to.be.revertedWith("Address already exists in this criterial");
  // });
  //============================================================================================================
  // transfer
