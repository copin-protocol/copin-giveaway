...
describe("Reward contract", function () {
  ...
  it("Should delete address from eligible Addresses", async function () {
    const { rewardContract, addr1, addr2 } = await loadFixture(
      deployRewardFixture
    );
    ...
    // Verify that addr2 is no longer eligible
    expect(await rewardContract.isEligible(addr2.address, 1)).to.equal(false);
  });
