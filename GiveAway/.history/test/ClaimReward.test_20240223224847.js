...
describe("Reward contract", function () {
  ...
  it("Should allow 200 eligible users to claim reward", async function () {
    const { rewardContract, rewardTokenContract, total } = await loadFixture(deployRewardFixture);
    ...
    // Generate 200 addresses and add them to the criterial
    for (let i = 0; i < 200; i++) {
        const wallet = ethers.Wallet.createRandom();
        addresses.push(wallet.address);
        await rewardContract.addCriterial(label, [wallet.address], unitReward);
    }

    // Claim reward for each address
    for (let i = 0; i < 200; i++) {
        claims.push(rewardContract.connect(addresses[i]).claimReward(1));
    }

    // Wait for all claims to be processed
    await Promise.all(claims);

    // Verify that each address received the correct reward
    for (let i = 0; i < 200; i++) {
        const balanceOfAddress = await rewardTokenContract.balanceOf(addresses[i]);
