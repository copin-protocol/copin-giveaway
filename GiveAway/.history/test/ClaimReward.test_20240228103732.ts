...
describe("Reward contract", function () {
  ...
  it("Should emit AddressRemoved event", async function () {
    // Add a criterial with addr1 and addr2 as eligible
    const unitReward = 100;
    await giveAway.addCriterial([addr1.address, addr2.address], unitReward);
  
    // Delete address 2 from eligible Addresses
    const tx = await giveAway.removeAddress(addr2.address, 1);
  
    // Đợi đến khi giao dịch được xác nhận
    await tx.wait();
  
    // Kiểm tra xem sự kiện AddressRemoved đã được kích hoạt với đúng địa chỉ và criterialId
    expect(tx)
      .to.emit(giveAway, "AddressRemoved")
