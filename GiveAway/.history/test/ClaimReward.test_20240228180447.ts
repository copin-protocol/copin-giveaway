...
describe("Reward contract", function () {
  ...
  it("Should revert if Invalid user address", async function () {
    const { rewardContract, addr1, unitReward } = await loadFixture(
      deployRewardFixture
    );
    await rewardContract.addCriterial(addr1.address,uint)
    expect(await rewardContract.isEligible(123,1),)
  });
