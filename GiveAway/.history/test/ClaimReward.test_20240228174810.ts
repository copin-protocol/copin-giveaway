...
describe("Reward contract", function () {
  ...
  it("Should allow only owner to addAddressToCriterial", async function () {
    const { rewardContract, addr1, addr2,unitReward } = await loadFixture(deployRewardFixture);
  ...
  // isEligible 
  it("Should return false if the user address is invalid", async function () {
    const { rewardContract, addr1 } = await loadFixture(deployRewardFixture);

    // Call isEligible with an invalid user address
    const isEligible = await rewardContract.isEligible(0, 1);

    expect(isEligible).to.be.false;
  });

  it("Should return false if the criterial does not exist", async function () {
      const { rewardContract, addr1,unitReward } = await loadFixture(deployRewardFixture);

      await rewardContract.addCriterial(addr1.address,unitReward)
      // Call isEligible with a non-existent criterial ID
      const isEligible = await rewardContract.isEligible(addr1.address, 2);

      expect(isEligible).to.be.false;
  });

  it("Should return false if the user is not eligible for the criterial", async function () {
      const { rewardContract, addr1, addr2 } = await loadFixture(deployRewardFixture);

      // Add a criterial with addr1 as the only eligible address
      await rewardContract.addCriterial([addr1.address], 100);

      // Call isEligible with addr2, which is not eligible for the criterial
      const isEligible = await rewardContract.isEligible(addr2.address, 1);

      expect(isEligible).to.be.false;
  });

  it("Should return true if the user is eligible for the criterial", async function () {
      const { rewardContract, addr1 } = await loadFixture(deployRewardFixture);

      // Add a criterial with addr1 as the only eligible address
      await rewardContract.addCriterial([addr1.address], 100);

      // Call isEligible with addr1, which is eligible for the criterial
      const isEligible = await rewardContract.isEligible(addr1.address, 1);
